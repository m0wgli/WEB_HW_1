...
def add_note():
    tag = Keyword(input(
        "Введи теги через кому або залиш поле порожнім:\n>>> "))
...
def del_note():
    param = input(
        "Введи тег для видалення нотатки:\n>>> ")
    return NOTE_BOOK.delete_note(param)


def search_note():
    param = input(
        "Введи слово для пошуку у нотатнику:\n>>> ")
...
def edit_keyword(
    notebook: NOTE_BOOK, note_name: str, old_keyword: str, new_keyword: str
) -> str:
    for key, record in notebook.data.items():
        if note_name == key:
...
def add_contact():
    name = Name(
        input("Введи ім'я користувача>>> ").capitalize())
    phone = Phone(input(
        "Введи номер телефону користувача починаючи с +38>>> "))
    rec = Record(name=name, phone=phone)
    return ADRESS_BOOK.add_record(rec)


def print_header_contact():
    name, phone, email, adres, bd = (
        "Ім'я",
        "Телефон",
        "E-mail",
        "Адреса",
...
def search_contact():
    search_input = input(
        "Введи ім'я або телефон для пошуку\n>>> ").capitalize()
...
def show_contact(record: Record):
    ...
    email = record.email.value if record.email else ""
    adres = record.address.value if record.address else ""
    bd = (record.birthday.value).strftime(
...
def delete_contact_record():
    name = input(
        "Введи ім'я контакту, котрого треба видалити\n>>> ").capitalize()
...
>>> "
    ...
    match add_input:
        case "phone":
            user_input = input(
                "Введи номер телефону у форматі +380123456789\n>>> ")
            return values.add_phone(user_input)
        case "email":
            user_input = input(
                "Введи електронну пошту\n>>> ")
            return values.add_email(user_input)
        case "bd":
            user_input = input(
                "Введи день народження контакту у форматі дд-мм-рррр\n>>> "
            )
...
>>> "
    ...
    match del_input:
        case "phone":
            user_input = input(
                "Введи номер для видалення у форматі +380123456789\n>>> "
            )
...
>>> "
    ...
    match change_input:
        case "phone":
            phone_input = (
                input(
                    "Через кому введи старий та новий номер телефону у форматі +380633456789,+380501234567\n>>> "
                )
            ).split(",")
            return values.change_phone(phone_input[0], phone_input[1])
        case "email":
            email_input = input(
                "Введи імейл користувача\n>>> ")
...
def edit_contact():
    contact_input = input(
        "Введи ім'я контакта, дані якого хочеш змінити\n>>> "
    ).capitalize()
...
class Interface:
    ...
    >>> "
            ...
            match user_input:
                ...
                case "search":
                    search_contact()
                case "show":
                    Interface.show_all_contacts()
                case "menu":
                    ADRESS_BOOK.save_to_bin()
                    main()
                case _:
                    print(
                        "Вибач, але в записній книзі немає такої команди.")
    ...
    >>> "
            ...
            match user_input:
                ...
                case "edit":
                    user_input = input(
                        "Що саме ти хочеш змінити? (тег чи текст нотатки)>>>"
                    )
                    match user_input:
                        case "тег":
                            name = input("name>>")
                ...
                case _:
                    print(
                        "Вибач, але в нотатнику поки що немає такої команди.")
...
COMMANDS = {
    help: "help",
    Interface.note_book: "notes",
    sorter: "sort",
    close: "exit",
    Interface.adress_book: "contacts",
